...
<body>
    ...
    <header class="text-center text-light masthead" style="background : url('assets/img/bg-masthead.jpg')no-repeat center center; backgroud-size: cover;">
        <div class="container">
            ...
            <div class="row mb-3">
                <?php
                    if('POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['weton'])){
                        ...
                        $hari = array("Ahad", "Sabtu", "Jum'at", "Kamis", "Rabu", "Selasa", "Senin");
                        $neptu = array("Legi", "Kliwon", "Wage", "Pahing", "Pon");
                        $index= 0;
                        $index1=0;
                        $pasaran = [];
                        for($i=0;$i<35;$i++){
                            $pasaran[$i]= $hari[$index]." ".$neptu[$index1];
                            if($index==6){
                                $index=0;
                            }else{
                                $index++;
